...
//Xem danh sách các bài viết theo categoryID
    ...
    @GetMapping("/search")
    public ResponseEntity<List<Post>> search(@RequestParam("author") String name) {
        List<Post> posts=service.getPostByName(name);
        if (posts.isEmpty()) {
            return new ResponseEntity<>(HttpStatus.NO_CONTENT);
        }
        return new ResponseEntity<>(posts,HttpStatus.OK);
    ...
    @GetMapping("/page")
    public ResponseEntity<Page<Post>> page(@RequestParam int page, @RequestParam int size) {
        Page<Post> posts=service.getPostsPage(PageRequest.of(page - 1,size));
        return new ResponseEntity<>(posts,HttpStatus.OK);
    }
